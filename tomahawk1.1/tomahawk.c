...
Usage(char *cmd)
{
    ...
    fprintf(stderr, "    -R rate         playback rate in Mbps (default:"
    		    " unlimited\n");
    ...
    fprintf(stderr, "    -L logFile      Run in logMode and log packets that time out to logFile (- is STDOUT).\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Options individual streams (handlers):\n");
    fprintf(stderr, "    -A (0|1)        Modify IP addresses (default: true)\n");
    fprintf(stderr, "    -d              Randomize lower 2 bytes of IP address (use only when the pcap has 2 IPs in it.)\n");
...
void
GetEther (char *device, Mac eaddr)
{
    char tmpName[256];
    char buf[256];
    FILE *file;
...
void
OpenInterface (char *device, Interface *rv)
{
    int fd;
    struct ifreq ifr;
...
inline
int
WriteInterface (Interface *interface, unsigned char *payload, int len)
{

    numSent++;
    return sendto(interface->fd, payload, len,
...
void
CloseInterface (Interface *interface)
{
    close (interface->fd);
 ...
 */
void
CreatePacketSendGroup (Handler *h)
{
     ...
     */
    if ( ! lowestIdUpdated ) h->lowestUnsentPktId = nextPktId;
	...
	for ( i=0; i<h->numFlowsInGroup ; i++ ) {
	     ...
	     */
	    if ( h->numPktsSent >= MAX_PACKETS_OUTSTANDING ) break;

	    /*
	     * Skip flows where we've already seen the other host
	     */
	    if ( h->flowsOutStateTable[ h->flowsOut[i] ].otherHostSeen ) continue;
...
void
FinishHandler(Handler *h, int timeout)
{
    Trace *t;
    Handler *prev, *curr;
...
void
LoadPacket (
    u_char *user,                    /* (IN/OUT) packet trace */
    ...
    const u_char * data)
{
    ...
    if (ph->ether_type == htons(ETH_P_IP) || ph->ether_type == htons(0x8100)){
    }
    else
        return;
    ...
    /* Get the IP header */ 
    if (ph->ether_type==htons(0x8100))
    memcpy (&iph, data+14+4, sizeof(iph));
        else
	...
	if ( debugFlag ) {
	    ...
	    if ( logFile == (FILE *) NULL ) {
		printf("LoadPacket: Skipping protocol Id %d, src: %s, ",
				iph.protocol, inet_ntoa(srcIP) );
     ...
     */
    if (ph->ether_type==htons(0x8100))
        pkt->len = ntohs(iph.tot_len) + 14 + 4;
    else
    ...
    if ( logPacket == 1 ) LogPacketInfo(pkt, "is truncated");
...
Trace *
LoadTrace (char *name, int loop, int maxActive, int timeout, int retrans,
	   int modAddrs)
{
    pcap_t *in_file;
    Trace *trace;
    ...
    if (pcap_dispatch(in_file, -1, (void *)&LoadPacket,
		      (u_char *) trace) == -1) {
	   ...
	   const u_char *packetData)
{
	...
	for ( i=0; i<h->numPktsSent ; i++ ) {
		...
		if (!PacketEqual(&t->pkt[pktId], packetData, pcap_hdr->caplen,
		    pcap_hdr->len)) {
		    continue;
		}
	    }
...
SendPackets(void *data)
{
	 ...
	 */
    struct ether_header *ph;
    ph = (struct ether_header *) p[pktId].buffer;
    if (ph->ether_type==htons(0x8100))
	memcpy (&iph, p[pktId].buffer+14+4, sizeof(iph));
    else
	memcpy (&iph, p[pktId].buffer+14, sizeof(iph));
	hlen = 20+(iph.ihl<<2);
	assert (hlen < sizeof(save));
    if (ph->ether_type==htons(0x8100))
        memcpy (&save, p[pktId].buffer+14+4, hlen);
    else
	...
	if (t->modAddrs) {
        ...
        if (ph->ether_type==htons(0x8100))
            memcpy (p[pktId].buffer+14+4, &iph, sizeof(iph));
	    else
    ...
    if (ph->ether_type==htons(0x8100))
        memcpy (p[pktId].buffer+14+4, &save, hlen);
    else
...
void
LogPacketInfo (Packet *pkt, char *msg)
{
    struct ether_header *ph;
    struct iphdr iph;
    struct tcphdr tcph;
    ...
    ph = (struct ether_header *) pkt->buffer;
    if (ph->ether_type==htons(0x8100))
        memcpy (&iph, pkt->buffer+14+4, sizeof(iph));
    else
...
CanRewriteIP (struct iphdr *iph)
{
	 ...
	 */
	if ( ( iph->saddr & 0xFF ) == 0xFF ) return 0;	/* highest byte is 255	*/
	if ( ( iph->daddr & 0xFF ) == 0xFF ) return 0;	/* 255.X.X.X		*/

	if ( ( iph->saddr & 0xFF ) == 0x00 ) return 0;	/* highest byte is 0	*/
	if ( ( iph->daddr & 0xFF ) == 0x00 ) return 0;	/* 0.X.X.X		*/
    ...
    fflush(stdout);
		...
		if (h->retrans == 0) {
			...
			if (t->modAddrs) {
			    ...
			    charIP.s_addr= GetWireIP(h->id, p[unrecvPktId].wireSrc, p[unrecvPktId].saddr, firstByte);
			    sprintf(buffer," (Wire IP: src %s, ",inet_ntoa(charIP));
			    strcat(message, buffer);
			    charIP.s_addr = GetWireIP(h->id, p[unrecvPktId].wireDst, p[unrecvPktId].daddr, firstByte);
			    sprintf(buffer,"dst %s)", inet_ntoa(charIP));
			    strcat(message, buffer);
			}
			
			if (!quiet) {
			    printf ("Expected Packet Not Received (pktId %u of %u).\n",unrecvPktId,t->numPkts);
			}
			...
			if (debugFlag) {
			    printf ("List of expected Packet(s) that were not received in the send group that timed out:\n");
			    int i;
			    for ( i=0 ; i < h->numPktsSent ; i++ ) {
				unsigned int pktId = h->pktsOut[i];
...
main(int argc, char *argv[])
{
	...
	case 'N':
	    ...
	    if ( maxActive > MAX_ACTIVE_HANDLERS ) maxActive = MAX_ACTIVE_HANDLERS;
	...
	case 'm':
	    ...
	    if ( maxOutstanding > MAX_PACKETS_OUTSTANDING ) maxOutstanding = MAX_PACKETS_OUTSTANDING;
	    break;
	case 'n':
	...
	case 'w':
	    ...
	    if ( maxPcapLookAhead > MAX_TRACE_LOOKAHEAD ) maxPcapLookAhead = MAX_TRACE_LOOKAHEAD;
	    break;
    default:
	Usage(argv[0]);
      }
	...
	fprintf(stderr,"\t( end ID - start ID ) >= Number Simultaneous Active Handlers (i.e. %u - %u >= %d )\n\n",
		endHandlerId, startHandlerId, maxActive);
	fprintf(stderr,"\tEither limit the number of loops to replay (see '-l')\n\tor limit the maximum number of simultaneous handlers (see '-N').\n\n");
	Usage(argv[0]);
    }
	...
	trace = LoadTrace (fileInfo[files].name,
			   ...
			   fileInfo[files].timeout,
			   fileInfo[files].retrans,
			   fileInfo[files].modAddrs);
	CreateHandler (trace);
    }
